...
create INDEX IF NOT EXISTS idx_timestamp_bid on librecs (timestamp, bid);
create INDEX IF NOT EXISTS idx_time_bookid on libreviews (Time DESC, BookId);

-- Покрывающий индекс для агрегации рейтингов (ускорит GROUP BY в 5-10 раз)
create INDEX IF NOT EXISTS idx_bookid_rate_id on librate (BookId, Rate, id);
